"""Authentication endpoints — anonymous + Firebase."""
...
from app.infrastructure.auth.jwt import create_token, get_or_create_user
...
async def anonymous_auth(
    ...
    """Authenticate with device ID. Creates a new user if needed."""
    device_id = request.headers.get("X-Device-ID") or str(uuid.uuid4())
    user = await get_or_create_user(device_id, db)
    token, expires_at = create_token(user.id, device_id)
